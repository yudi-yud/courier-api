...
func (s *pdfService) GenerateAirwayBill(shipment *models.Shipment) ([]byte, error) {
	...
	pdf.SetFont("Arial", "", 8)
	pdf.MultiCell(0, 5, fmt.Sprintf("%s\n%s", shipment.SenderName, shipment.SenderAddress), "", "L", false)
	pdf.Ln(3)
	...
	pdf.SetFont("Arial", "", 8)
	pdf.MultiCell(0, 5, fmt.Sprintf("Phone: %s", shipment.ReceiverPhone), "", "L", false)
	...
	pdf.CellFormat(0, 5, fmt.Sprintf("Created at: %s", shipment.CreatedAt.Format("2006-01-02 15:04:05")), "", 0, "C", false, 0, "")
